...
function Dashboard(){
    ...
    return <div>
        <Sidebar/>
        <div className="px-3 ml-72 min-h-screen">
            <AddContentModel open={modalOpen} onClose={() => {setmodalOpen(false)}}/>
            <div className="flex pt-3 justify-end gap-2">
            <Button startIcon={<ShareIcon size="md"/>} size= "sm" variant="primary" text="Share"></Button>
            <Button onClick={() => { setmodalOpen(true) }}startIcon={<PlusIcon size="md"/>} size= "sm" variant="secondary" text="Add Content"></Button>
            </div>
